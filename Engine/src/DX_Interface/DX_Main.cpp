...
// quary for a supported graphics addaptor
ComPtr<IDXGIAdapter4> GetAdapter(bool useWarp)
{
    ...
    else
    {
        ...
        for (UINT i = 0; dxgiFactory->EnumAdapters1(i, &dxgiAdapter1) != DXGI_ERROR_NOT_FOUND; ++i)
        {
            ...
            // creating it.(null device) The adapter with the largest dedicated video memory
            // is favored.
...
ComPtr<ID3D12Device2> CreateDevice(ComPtr<IDXGIAdapter4> adapter)
{
    ...
    if (SUCCEEDED(d3d12Device2.As(&pInfoQueue)))
    {
        ...
        // ignore messages based on their category
        //D3D12_MESSAGE_CATEGORY Categories[] = {};

        // ignore messages based on their severity level
        D3D12_MESSAGE_SEVERITY Severities[] =
        {
            D3D12_MESSAGE_SEVERITY_INFO
        };

        // ignore specific individual messages by their ID
        D3D12_MESSAGE_ID DenyIds[] = {
            D3D12_MESSAGE_ID_CLEARRENDERTARGETVIEW_MISMATCHINGCLEARVALUE,   // when clearing the render taget with a color thats not the optimized clear color value.
...
// used for storing and managing resource views
...
{
    // get vendor specific descriptor size
...
// used to allocate memory for command lists
...
void Render()
{
    ...
    {
        // first transition to render target state
...
void Resize(uint32_t width, uint32_t height)
{
    if (g_ClientWidth != width || g_ClientHeight != height)
    {
        ...
        // flush the gpu so the swapchains back buffers are not refrenced
